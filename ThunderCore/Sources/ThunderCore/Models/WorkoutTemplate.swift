...
public final class WorkoutTemplate {
    ...
    /// The type of activity this template is for.
    ///
    /// Optional — a template may be saved without an activity type selected.
    /// Delete rule is `.nullify` — removing an `ActivityType` does not delete
    /// the templates that referenced it.
    @Relationship(deleteRule: .nullify, inverse: \ActivityType.templates)
